...
## 7. CI 构建库 schema 修复（R4）— 2026-08-05 追加

> 处置：`.cn` 首次 CI 真跑时 `npm run build` 失败（ISR 预渲染查询空库缺表）。
> 分支：`cn-ci-schema`（commit `77a1c02`，基于 main 5eb70c2；**未 push、未开 PR**）。
> ⚠️ 环境限制：本机无法创建 `refs/heads/feat/` 嵌套目录（嵌套 ref 会被外部进程删除，
> 单层 ref 正常），故分支名为 `cn-ci-schema`。合入时可 `git branch -m cn-ci-schema feat/cn-ci-schema` 还原。

### 7.1 失败现象与根因

- **现象**：merge 到 main 后 deploy-cn.yml 的 `npm run build` 步骤失败：
  ```
  miniapp brands-categories error: PrismaClientKnownRequestError:
  Invalid prisma.category.findMany() invocation:
  The table `public.Category` does not exist in the current database.
  ```
  发生在 next build "Generating static pages (26/53)" 阶段——ISR 预渲染真实连库查询。
- **根因**：R3 的构建期临时 postgres service（`services.postgres`）只有空库**无表结构**；
  页面/路由（如 `src/app/api/miniapp/brands-categories/route.ts` 直接
  `prisma.brand.findMany()` + `prisma.category.findMany()`）查询即抛 P2021。
- **为什么用 db push**：`prisma/migrations/` 仅 2 个**增量**迁移（依赖历史基础表），
  对空库 migrate deploy 会失败；与 R1 部署期一致，用 `prisma db push` 全量建表（幂等）。

### 7.2 改动（仅 1 个文件，+10 行）

`.github/workflows/deploy-cn.yml`：在 `Generate Prisma Client` 之后、`npm run build` 之前插入：

```yaml
      - name: Init build database schema
        # R4 修复：构建期临时 postgres service 只有空库无表，ISR 预渲染真实连库查询
        # （如 miniapp/brands-categories 的 prisma.category.findMany）会因
        # "table does not exist"（P2021）失败。用 prisma db push 按仓库 schema 建好
        # 全部表结构（空表无数据），与 R1 部署期做法一致；--skip-generate 不再重复生成
        # client（上一步已生成）。migrations 仅 2 个增量、依赖历史基础表，空库必须 db push。
        run: npx prisma db push --skip-generate
        env:
          DATABASE_URL: ${{ env.CI_DATABASE_URL }}
```

检查点：① 位置在 Generate Prisma Client 之后、npm run build 之前 ✅；② env 用
`CI_DATABASE_URL`（localhost:5432 指向 R3 postgres service）✅；③ service 端口已发布、可达 ✅；
④ `--skip-generate` 不重复生成 client ✅；⑤ services postgres（R3）与 OSS 上传段均未动 ✅。

### 7.3 验证结果

- `npx prisma@5.14.0 validate`：**通过**（`The schema ... is valid`；需设置 DATABASE_URL 环境变量，
  validate 不实际连库）。
- workflow YAML 结构：step 插入位置与 env 逐行核对正确（Generate 步骤 env 之后、Build 步骤之前）。
- 未跑 docker/next build（按要求轻量验证）；CI 端效果以合入后真跑为准。

### 7.4 ISR 空库行为结论（16 个 revalidate 页面清单）

构建期真实连库预渲染的页面（`export const revalidate`）共 **16 个**：
- 首页 `[locale]/page.tsx`(300)、`products/page.tsx`(300)
- `category/[slug]`(600)、`category/spare-parts`(600)、`category/pickup-header`(600)、`category/forage-header`(600)
- `brand/[slug]`(600)、`blog/page.tsx`(3600)
- `expo/compare`(300)、`expo/china-brands`(300)、`expo/global-brands`(300)、`expo/showroom`(300)、`expo/showroom/[id]`(300)

另：`force-dynamic` 页（admin 等）构建期不查库，无影响。

**db push 建表后（空库）行为**：所有查询返回**空数组**，不再抛 P2021；首页等已 try/catch 兜底
（`[HomePage] 文章查询失败` 返回 []），其余 ISR 页渲染空列表 → **构建可通过**。

⚠️ 注意：构建产物为**空数据快照**（ISR revalidate 300~3600s 后运行时才从 ECS 库刷新）。
`.cn` 上线需要内容仍依赖 R2 基础数据 seed（U8），本次修复只解决"能构建"，不解决"有内容"。
