...
async function callOnce(sources, srcLang, tgtLang) {
  ...
  const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: MODEL, temperature: 0.1, max_tokens: 4000,
...
async function translateBatch(sources, { srcLang, tgtLang }) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try { return await callOnce(sources, srcLang, tgtLang); }
      ...
      if (e.status === 429 || (e.status >= 500)) {
        process.stderr.write(`  [warn] ${e.message} retry ${attempt + 1} after ${backoff | 0}ms\n`);
        await new Promise(r => setTimeout(r, backoff));
        continue;
      }
      if (String(e.message).startsWith("PARSE_FAIL")) {
        process.stderr.write(`  [warn] parse fail retry ${attempt + 1}\n`);
  ...
  for (const s of sources) {
    for (let a = 0; a < 3; a++) {
      try { const r = await callOnce([s], srcLang, tgtLang); out.push(r[0]); break; }
      catch (e) { lastErr = e; if (a === 2) out.push(s); else await new Promise(r => setTimeout(r, 500)); }
    }
  }
  return out;
}
