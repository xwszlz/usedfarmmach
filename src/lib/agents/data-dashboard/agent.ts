...
export class DataDashboardAgent {
  ...
  /**
   * 每日流水线四段：schedule=当前实际 cron，target=目标 4 段式。
   * schedule 与 target 均写 UTC cron；北京时区解释见 note。
   * 已如实标注「已对齐 / 待对齐」，不虚报对齐状态。
   */
  private readonly PIPELINE = [
    { key: "seller-scout", label: "① 卖方采集", schedule: "0 22 * * *", target: "0 22 * * *", source: "GitHub Actions", note: "每日 06:00 北京（UTC 22:00）。GitHub Actions schedule 常有 1~1.5h 延迟，实际约 07:30 后。已对齐" },
    { key: "price-intel", label: "② 国际价格刷新", schedule: "0 23 * * 0", target: "30 22 * * *", source: "Vercel Cron", note: "当前仅周一 07:00 北京（UTC 周日 23:00）；目标每日 06:30 北京（UTC 22:30）。待对齐" },
    { key: "brand-benchmark", label: "③ 品牌基准价", schedule: "30 23 * * 0", target: "0 23 * * *", source: "Vercel Cron", note: "当前仅周一 07:30 北京（UTC 周日 23:30）；目标每日 07:00 北京（UTC 23:00）。待对齐（18 品牌 × 7 来源）" },
    { key: "daily-report", label: "④ AI 日报", schedule: "45 23 * * *", target: "45 23 * * *", source: "Vercel Cron", note: "每日 07:45 北京（UTC 23:45）。本次新增，排在基准价刷新之后。已对齐" },
  ] as const;
