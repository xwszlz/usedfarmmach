...
export default function SellerAiAssistant({ imageFiles = [], imageUrls: propImageUrls, videoFile = null, onFill, autoTrigger = true, editMode = false, existingFormValues, }: SellerAiAssistantProps) {
    const tr = useTr();
    ...
    const handleRecognize = async () => {
        ...
        try {
            ...
            if (isUrlMode) {
                ...
                const imageDataUrls = await Promise.all(bestImages.map(async (file) => {
                    try {
                ...
                if (uploadFailCount > 0) {
                    console.warn(`[AI] ${uploadFailCount}/${imageDataUrls.length} 张图片上传失败，剩余 ${finalImageUrls.length} 张继续识别`);
                }
                setPhase(`AI 智能识别中...(${finalImageUrls.length}张图片)`);
            }
    ...
    const renderFieldWithCheckbox = (formKey: string) => {
        ...
        // AI无值 → 灰色不可勾选
        if (!aiDisplayValue) {
            return (<div key={formKey} className="flex items-center justify-between text-sm text-gray-400">
        ...
        const existingValue = existingFormValues?.[formKey];
        const hasExisting = existingValue != null &&
            existingValue !== "" &&
            existingValue !== 0 &&
        ...
        return (<div key={formKey} className="flex items-center justify-between text-sm">
        <label className="flex cursor-pointer items-center gap-2">
          ...
          {isConflict && (<span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700">{tr("将覆盖现有值")}</span>)}
          {recognized.confidence < 0.6 && (<span className="rounded bg-orange-100 px-1.5 py-0.5 text-[10px] font-medium text-orange-600">{tr("低置信度")}</span>)}
        </label>
        <span className={`font-medium ${isChecked ? "text-green-700" : "text-gray-400"}`}>
          {displayValue}
        </span>
      </div>);
    };
    return (<div className="rounded-xl border border-primary-200 bg-gradient-to-br from-primary-50 to-white p-5 shadow-sm">
      <div className="mb-4 flex items-center gap-2">
        <Sparkles className="h-5 w-5 text-primary-600"/>
        <h3 className="text-base font-bold text-gray-900">{tr("AI 智能识别")}</h3>
        <span className="rounded-full bg-primary-100 px-2 py-0.5 text-[10px] font-medium text-primary-700">{tr("已上传")}{totalImages}{tr("张")}</span>
      </div>
      ...
      {/* 引擎模式选择器 */}
      <div className="mb-4 flex items-center gap-2">
        <span className="text-xs font-medium text-gray-600">{tr("识别引擎：")}</span>
        <div className="flex gap-1.5">
          <button onClick={() => setEngineMode("auto")} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${engineMode === "auto"
            ? "bg-primary-600 text-white"
            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            <Globe className="mr-1 inline h-3 w-3"/>{tr("自动")}</button>
          <button onClick={() => setEngineMode("domestic")} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${engineMode === "domestic"
            ? "bg-red-600 text-white"
            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            <Flag className="mr-1 inline h-3 w-3"/>{tr("国内农机")}</button>
          <button onClick={() => setEngineMode("international")} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${engineMode === "international"
            ? "bg-blue-600 text-white"
            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            <Globe className="mr-1 inline h-3 w-3"/>{tr("国际农机")}</button>
        </div>
      </div>

      {/* 自动识别温和提示条（仅在自动触发且识别中时显示） */}
      {autoRecognizing && recognizing && (<div className="mb-3 flex items-center gap-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-600">
          <Sparkles className="h-4 w-4 flex-shrink-0"/>{tr("已上传图片，AI 正在自动识别参数，您可先填写其他信息")}</div>)}

      {/* 无图片但有视频时的友好提示 */}
      {totalImages === 0 && videoFile && (<div className="mb-3 flex items-center gap-2 rounded-lg bg-amber-50 p-3 text-sm text-amber-600">
          <AlertCircle className="h-4 w-4 flex-shrink-0"/>{tr("请上传农机图片以启用 AI 识别（视频可作为补充）")}</div>)}

      {/* 识别按钮 */}
      <button onClick={handleRecognize} disabled={recognizing || totalImages === 0} className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary-600 px-4 py-3 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-50">
        {recognizing ? (<>
            <Loader2 className="h-4 w-4 animate-spin"/>
            {autoRecognizing ? phase || "AI \u6B63\u5728\u81EA\u52A8\u8BC6\u522B..." : phase || "\u8BC6\u522B\u4E2D..."}
          </>) : recognized ? (<>
            <Wand2 className="h-4 w-4"/>{tr("重新识别 (")}{totalImages}{tr("张图片)")}</>) : (<>
            <Wand2 className="h-4 w-4"/>{tr("开始智能识别 (")}{totalImages}{tr("张图片)")}</>)}
      </button>

      {/* 错误提示 */}
      {error && (<div className="mt-3 flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-600">
          <AlertCircle className="h-4 w-4 flex-shrink-0"/>
          {error}
        </div>)}

      {/* 识别结果 */}
      {recognized && (<div className="mt-4 rounded-lg border border-green-200 bg-green-50/50 p-4">
          <div className="mb-3 flex items-center gap-2">
            <CheckCircle className="h-4 w-4 text-green-600"/>
            <span className="text-sm font-medium text-green-800">{tr("识别成功（置信度")}{Math.round(recognized.confidence * 100)}%）
            </span>
            {recognized.isChineseBrand !== undefined && (<span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${recognized.isChineseBrand
          ...
          {/* 参考价格（仅展示，不回填表单价格输入框） */}
          {recognized.referencePrice != null && recognized.referencePrice > 0 && (<div className="mt-2 flex items-center justify-between rounded-md bg-amber-50 px-3 py-1.5 text-sm">
              <span className="text-gray-500">{tr("参考价格")}</span>
              <span className="font-medium text-amber-700">
                ¥{recognized.referencePrice.toLocaleString()}{tr("元")}<span className="ml-1 text-[10px] text-gray-400">{tr("（仅供参考，需手动填写）")}</span>
              </span>
            </div>)}

          {/* 选择性填充按钮（方案B） */}
          <button onClick={handleSelectiveFill} disabled={selectedFields.size === 0} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-green-600 bg-green-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50 disabled:hover:bg-green-600">
            <Upload className="h-4 w-4"/>{tr("填充已选字段（")}{selectedFields.size}{tr("项）")}</button>
        </div>)}
    </div>);
}
