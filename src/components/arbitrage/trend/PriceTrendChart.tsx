...
export default function PriceTrendChart({ data, title, height = 300 }: PriceTrendChartProps) {
  const tr = useTr();
    ...
    if (data.length === 0) {
        return (<div className="flex items-center justify-center h-full min-h-[200px] bg-gray-50 rounded-lg">
      ...
      <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
        <ResponsiveContainer width="100%" height={height}>
          <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Legend />
            <Line type="monotone" dataKey="domesticPrice" name={t("table.domesticPrice")} stroke="#3b82f6" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }}/>
            <Line type="monotone" dataKey="internationalPrice" name={t("table.internationalPrice")} stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }}/>
            <Line type="monotone" dataKey="priceDifference" name={t("table.priceDifference")} stroke="#8b5cf6" strokeWidth={1.5} strokeDasharray="5 5" dot={{ r: 3 }} activeDot={{ r: 5 }}/>
          </LineChart>
        </ResponsiveContainer>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 p-3 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">{tr("最新数据")}</p>
          </div>
          ...
          <div className="bg-purple-50 p-3 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">{tr("当前价差")}</p>
          </div>
        </div>
      </div>
    </div>);
