...
/**
 * 首页展会宣传 / 祝贺横幅（天津 CIAME 2026 · 单展版）
 *
 * 沿革：本组件由首页「双展横幅」收编而来。原版同屏展示两场展会（哈尔滨 + 天津），
 * 并带哈尔滨场的报名/开幕倒计时。哈尔滨场（2026-09-19~21，哈尔滨）已收官且不再推进：
 * 该场专题页及其攻略子页已下线，旧 URL 由 next.config.js 统一 301 到天津页。
 * 故此处收为单展，倒计时改挂天津开幕。
 *
 * 合规红线（沿用原稿）：
 * - 全文零「招展」，统一「诚邀入驻」
...
const TEXTS: Record<string, BannerCopy> = {
  zh: {
    congrats: "🎉 祝贺 2026 中国国际农业机械展览会（CIAME）即将盛大启幕",
    congratsShort: "🎉 2026 中国国际农机展 10/26–28 天津启幕",
    header: "神雕农机展™ · 2026 天津展会季",
    globalLine:
    ...
    countdown: {
      opening: (d, h) => `距开幕仅剩 ${d} 天 ${h} 小时（10月26日开幕）`,
      open: "展会已开幕 · 10月26–28日进行中",
    },
    ctaBar:
  ...
  en: {
    congrats:
      "🎉 Congratulations to the 2026 China International Agricultural Machinery Exhibition (CIAME) — opening soon",
    congratsShort: "🎉 CIAME 2026 · Tianjin, Oct 26–28",
    header: "Shendiao Agri-Machinery Expo™ · 2026 Tianjin Season",
    globalLine:
    ...
    countdown: {
      opening: (d, h) => `Opens in ${d}d ${h}h (Oct 26)`,
      open: "Expo is live · Oct 26–28",
    },
    ctaBar:
  ...
  ru: {
    congrats:
      "🎉 Поздравляем с предстоящим открытием Китайской международной выставки сельхозтехники (CIAME) — 2026!",
    congratsShort: "🎉 CIAME 2026 · Тяньцзинь, 26–28 октября",
    header: "Shendiao Agri-Machinery Expo™ · Сезон выставок 2026 в Тяньцзине",
    globalLine:
    ...
    countdown: {
      opening: (d, h) => `До открытия: ${d} дн. ${h} ч (26 октября)`,
      open: "Выставка открыта · 26–28 октября",
    },
    ctaBar:
...
/**
 * .com 全球跨境叙事覆盖：仅 .com 生效，标题行切换为全球舞台叙事；
 * .cn 保留国内入驻叙事（沿用 TEXTS[locale].header）。
 */
const GLOBAL_NARRATIVE: Record<string, { header: string; globalLine: string }> = {
  zh: {
    header: "一个全球舞台 · 2026 中国国际农机展",
    globalLine:
      "一次上架，8 语种触达全球买家 · 海外买家 2300+ · 中东欧、中亚、东南亚、非洲、南美",
  },
  en: {
    header: "One global stage — CIAME 2026",
    globalLine:
      "List your machines once — reach buyers worldwide in 8 languages. 2,300+ overseas buyers · Central & Eastern Europe, Central Asia, Southeast Asia, Africa, South America.",
  },
  ru: {
    header: "Одна мировая площадка — CIAME 2026",
    globalLine:
      "Разместите технику один раз — и вас увидят покупатели по всему миру на 8 языках. Более 2 300 зарубежных покупателей · Центральная и Восточная Европа, Центральная Азия, Юго-Восточная Азия, Африка, Южная Америка.",
  },
};

// 天津展官方展期 2026-10-26 ~ 10-28。
// 展位申报窗口已于 2026-05-15 关闭（官方《申报说明》），故只做「开幕」倒计时，不做报名倒计时。
const TJ_OPENING = new Date("2026-10-26T09:00:00+08:00").getTime();

type CountdownPhase = "opening" | "open";
...
function computeCountdown(): CountdownState {
  const now = Date.now();
  if (now >= TJ_OPENING) return { phase: "open", days: 0, hours: 0 };
  const diff = Math.max(0, TJ_OPENING - now);
  return {
    phase: "opening",
...
export function ExpoBanner({ locale }: ExpoBannerProps) {
  const isCn = isCnSite();
  ...
  const isCounting = countdown?.phase === "opening";
  const footnote = !countdown
    ? t.tianjinFootnote
    : isCounting
      ...
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8 lg:py-4">
        ...
        <div className="mt-3 lg:mt-4">
          <Link
            ...
            aria-label={t.altTianjin}
            className="group block rounded-xl bg-white/10 p-3 ring-1 ring-white/20 backdrop-blur transition-all hover:bg-white/20 hover:ring-white/40 sm:p-4"
          >
            ...
            <h3 className="mt-2 text-sm font-bold leading-snug text-white sm:text-lg">
              <span className="sm:hidden">{t.tianjin.nameShort}</span>
              <span className="hidden sm:inline">{t.tianjin.name}</span>
            </h3>

            <div className="mt-1 hidden items-start gap-1.5 text-xs text-red-50 sm:flex">
              <MapPin className="mt-0.5 h-3 w-3 shrink-0" />
              <span>{t.tianjin.scale}</span>
            </div>
