...
export function CreditTransactionList({ locale }: CreditTransactionListProps) {
    ...
    return (<Card className="space-y-4 p-6">
      ...
      {list.length === 0 ? (<p className="py-8 text-center text-sm text-gray-400">{t("noRecords")}</p>) : (<div className="divide-y">
          {list.map((tx) => {
  const tr = useTr();
  return (<div key={tx.id} className="flex items-center justify-between py-3">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {new Date(tx.createdAt).toLocaleDateString(locale === "zh" ? "zh-CN" : locale)}
                </p>
              </div>
              <span className={`text-sm font-semibold ${tx.amount > 0 ? "text-green-600" : "text-red-500"}`}>
                {tx.amount > 0 ? `+${tx.amount}` : tx.amount}{tr("分")}</span>
            </div>);
})}
        </div>)}
    </Card>);
