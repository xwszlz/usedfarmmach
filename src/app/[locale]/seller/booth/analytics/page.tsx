...
export default function BoothAnalyticsPage() {
    ...
    if (!data)
        return <div className="py-20 text-center text-gray-500">{tr("暂无数据")}</div>;
    ...
    return (<div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-6">
        <Link href="/zh/seller/booth" className="mb-3 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft className="h-4 w-4"/>{tr("返回展位管理")}</Link>
        <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
          <BarChart3 className="h-6 w-6 text-blue-600"/>{tr("展位数据看板")}</h1>
      </div>
      ...
      <section className="mb-6 rounded-xl border bg-white p-5 shadow-sm">
        <h2 className="mb-4 flex items-center gap-2 font-bold text-gray-900">
          <Star className="h-5 w-5 text-yellow-500"/>{tr("浏览TOP 5 设备")}</h2>
        {data.topItems.length === 0 ? (<p className="py-6 text-center text-sm text-gray-400">{tr("暂无数据")}</p>) : (<div className="space-y-3">
            {data.topItems.map((item, idx) => (<div key={item.id} className="flex items-center gap-4 rounded-lg border p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-4 text-xs text-gray-400">
                    ...
                    <span className="flex items-center gap-1"><TrendingUp className="h-3 w-3"/>{tr("转化率")}{item.conversionRate}%</span>
                  </div>
                </div>
                {/* Bar chart visual */}
                <div className="hidden h-8 w-32 items-end gap-0.5 sm:flex">
                  {Array.from({ length: 20 }).map((_, i) => (<div key={i} className="flex-1 rounded-t bg-blue-200" style={{ height: `${Math.min(100, (item.viewCount / Math.max(...data.topItems.map(t => t.viewCount), 1)) * (i < (item.viewCount / Math.max(...data.topItems.map(t => t.viewCount), 1) * 20) ? 100 : 0))}%` }}/>))}
                </div>
              </div>))}
          </div>)}
      </section>
      ...
      {/* Inquiry Trend */}
      <section className="mb-6 rounded-xl border bg-white p-5 shadow-sm">
        <h2 className="mb-4 font-bold text-gray-900">{tr("询盘趋势（近30天）")}</h2>
        {data.inquiryTrend.length === 0 ? (<p className="py-6 text-center text-sm text-gray-400">{tr("暂无询盘数据")}</p>) : (<>
            <div className="flex items-end gap-1" style={{ height: 120 }}>
              {data.inquiryTrend.map(d => {
                const maxCount = Math.max(...data.inquiryTrend.map(t => t.count), 1);
                return (<div key={d.date} className="group relative flex-1" style={{ minWidth: 4 }}>
                    <div className="absolute bottom-0 w-full rounded-t bg-blue-500 transition group-hover:bg-blue-600" style={{ height: `${(d.count / maxCount) * 100}%` }}/>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <section className="rounded-xl border bg-white p-5 shadow-sm">
          <h2 className="mb-3 font-bold text-gray-900">{tr("询盘意向分布")}</h2>
          {Object.keys(data.intentStats).length === 0 ? (<p className="py-4 text-center text-sm text-gray-400">{tr("暂无数据")}</p>) : (<div className="space-y-2">
              {Object.entries(data.intentStats).map(([intent, count]) => {
                const total = Object.values(data.intentStats).reduce((a, b) => a + b, 0);
                const pct = total > 0 ? (count / total * 100).toFixed(0) : 0;
                return (<div key={intent}>
        ...
        <section className="rounded-xl border bg-white p-5 shadow-sm">
          <h2 className="mb-3 font-bold text-gray-900">{tr("询盘状态分布")}</h2>
          {Object.keys(data.statusStats).length === 0 ? (<p className="py-4 text-center text-sm text-gray-400">{tr("暂无数据")}</p>) : (<div className="space-y-2">
