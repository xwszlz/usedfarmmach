...
import { Factory, Tractor, Wheat, Sprout, Plane, Layers, ExternalLink, ChevronRight, Star, Award, Crown, Globe, Calendar, ArrowUpRight, Filter, Tag, MapPin, Sparkles, TrendingUp, } from "lucide-react";
interface Brand {
    id: string;
    nameZh: string;
    nameEn: string;
      ...
      <div className="relative p-5">
        ...
        <div className="mb-4 flex items-start gap-4">
          {brand.expoLogoUrl ? (<div className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gray-50 p-2 ring-1 ring-gray-100">
            ...
            </div>) : (<div className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${config.gradient} text-white shadow-md`}>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          {brand.establishedYear && (<span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2.5 py-1 text-xs font-medium text-gray-600">
              ...
              {brand.establishedYear}{tr("年创立")}</span>)}
        ...
        <div className="flex items-center justify-between border-t border-gray-50 pt-3">
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-red-600 transition-colors group-hover:text-red-700">{tr("进入展位")}<ArrowUpRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"/>
          </span>
          {brand.officialWebsite && (<a href={brand.officialWebsite} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 text-xs text-gray-400 transition-colors hover:text-gray-600">
              <ExternalLink className="h-3 w-3"/>{tr("官网")}</a>)}
        </div>
      </div>
    </Link>);
}
  ...
  const tr = useTr();
    ...
    return (<Link href={`/${locale}/expo/showroom/${item.id}`} className="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-44 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        {item.isFeatured && (<span className="absolute left-3 top-3 flex items-center gap-1 rounded-full bg-red-500 px-2.5 py-1 text-xs font-bold text-white shadow-md">
            <TrendingUp className="h-3 w-3"/>{tr("热销")}</span>)}
      ...
      <div className="p-4">
        <div className="mb-2 flex items-center gap-2">
          ...
          {item.brandRel?.brandTier === "flagship" && (<Crown className="h-3.5 w-3.5 text-red-500"/>)}
        </div>
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          {item.launchYear && (<span className="text-gray-400">{item.launchYear}{tr("款")}</span>)}
        </div>
        {(item.msrpCny || item.msrpUsd) && (<div className="mt-3 border-t border-gray-50 pt-3">
            {item.msrpCny && (<div className="text-lg font-extrabold text-red-600">
                ¥{item.msrpCny.toLocaleString()}
                <span className="ml-1 text-xs font-normal text-gray-400">{tr("起")}</span>
              </div>)}
            {item.msrpUsd && (<div className="text-xs text-gray-400">
...
export default function ChinaBrandsClient({ brands, items, categoryCounts, tierCounts, locale, }: {
    brands: Brand[];
    items: ShowcaseItem[];
  ...
  const tr = useTr();
    ...
    return (<div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50">
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-gradient-to-br from-red-700 via-red-800 to-rose-900 pb-20 pt-16 text-white">
        ...
        <div className="absolute inset-0 opacity-10">
          <svg className="absolute right-0 top-0 h-full w-1/2" viewBox="0 0 400 600">
            <path d="M200 100 Q300 150 350 250 Q400 350 300 450 Q200 550 100 450 Q0 350 50 250 Q100 150 200 100Z" fill="none" stroke="white" strokeWidth="0.5" opacity="0.3"/>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium backdrop-blur-md">
            ...
            <span>{tr("中国中坚馆 · 主馆")}</span>
            <span className="ml-1 rounded-full bg-white/20 px-2 py-0.5 text-xs">{tr("105品牌")}</span>
          </div>

          <h1 className="mb-4 text-4xl font-extrabold tracking-tight sm:text-5xl lg:text-6xl">{tr("中国农机品牌")}<span className="block bg-gradient-to-r from-yellow-300 to-amber-200 bg-clip-text text-transparent">{tr("走向世界")}</span>
          </h1>
          ...
          <p className="max-w-2xl text-lg leading-relaxed text-red-100">{tr("42个中国农机品牌，覆盖拖拉机、收割机、播种机、植保无人机全品类。2025年中国农机出口674亿元，同比增长32.3%——中国制造正在改变全球农业装备格局。")}</p>

          {/* Stats Cards */}
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 lg:gap-6">
            <StatCard number={String(brands.length)} label="中国品牌" sub="旗舰·核心·精选" icon={Factory} gradient="from-red-500 to-rose-600"/>
            <StatCard number="30+" label="旗舰机型" sub="最新技术前沿" icon={Tractor} gradient="from-amber-500 to-orange-600"/>
            <StatCard number="6" label="品类全覆盖" sub="从田间到天空" icon={Layers} gradient="from-blue-500 to-cyan-600"/>
            <StatCard number="674亿" label="年出口额" sub="2025年同比增长32.3%" icon={TrendingUp} gradient="from-green-500 to-emerald-600"/>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <span className="mr-2 flex items-center gap-1 text-sm font-medium text-gray-500">
              <Filter className="h-4 w-4"/>{tr("品牌等级")}</span>
            ...
            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{tr("全部 (")}{brands.length})
            </button>
  ...
  const tr = useTr();
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="mr-2 flex items-center gap-1 text-sm font-medium text-gray-500">
              <Tag className="h-4 w-4"/>{tr("产品品类")}</span>
            ...
            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{tr("全部")}</button>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{tr("品牌阵容")}</h2>
            <p className="mt-1 text-sm text-gray-500">{tr("共")}{filteredBrands.length}{tr("个品牌")}{selectedTier !== "all" && (<span className="ml-1 text-red-600">
                  · {getTierConfig(tr)[selectedTier].label}
                </span>)}
          ...
          {(selectedTier !== "all" || selectedCategory !== "all") && (<button onClick={() => {
            ...
            }} className="rounded-full bg-gray-100 px-3 py-1.5 text-sm text-gray-600 transition hover:bg-gray-200">{tr("清除筛选")}</button>)}
        </div>
        ...
        {filteredBrands.length === 0 && (<div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-lg font-medium">{tr("暂无符合条件的品牌")}</p>
            <p className="text-sm">{tr("尝试调整筛选条件")}</p>
          </div>)}
      </section>
      ...
      <section className="bg-gradient-to-b from-gray-50 to-white py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{tr("旗舰机型展示")}</h2>
              <p className="mt-1 text-sm text-gray-500">{tr("精选")}{(selectedCategory === "all" ? items : filteredItems).length}{tr("台热门机型")}</p>
            </div>
            <Link href={`/${locale}/expo/showroom`} className="inline-flex items-center gap-1 rounded-full bg-red-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-red-700">{tr("全部展厅")}<ChevronRight className="h-4 w-4"/>
            </Link>
          </div>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-red-600 to-rose-700 py-16">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center text-white sm:px-6 lg:px-8">
          <h2 className="mb-4 text-3xl font-bold sm:text-4xl">{tr("中国农机，全球机遇")}</h2>
          <p className="mx-auto mb-8 max-w-2xl text-lg text-red-100">{tr("从展示到成交，365天不间断。让您的中国农机品牌走向世界每一个田间。")}</p>
          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link href={`/${locale}/expo/showroom`} className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 font-semibold text-red-600 shadow-lg transition hover:bg-red-50 hover:shadow-xl">{tr("进入线上展厅")}<ChevronRight className="h-4 w-4"/>
            </Link>
            <Link href={`/${locale}/expo/global-brands`} className="inline-flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-6 py-3.5 font-semibold text-white backdrop-blur-sm transition hover:bg-white/20">{tr("查看国际品牌")}<Globe className="h-4 w-4"/>
            </Link>
          </div>
        </div>
      </section>
