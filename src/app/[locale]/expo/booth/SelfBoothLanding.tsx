...
export default function SelfBoothLanding({ locale }: SelfBoothLandingProps) {
    ...
    return (<div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="relative bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 text-white">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-8 lg:px-8">
          ...
          <div className="mt-10 grid grid-cols-3 gap-4 sm:grid-cols-6">
            {brandAssets.map((a, i) => (<div key={i} className="rounded-lg bg-white/10 p-3 text-center backdrop-blur-sm">
      ...
      <section className="border-b border-gray-200 py-12 dark:border-gray-800">
        ...
        ].map((item, i) => (<div key={i} className="rounded-xl border border-gray-200 p-6 dark:border-gray-800">
      ...
      <section className="py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 lg:grid-cols-4">
            {tiers.map((tier, i) => (<div key={i} className={`relative flex flex-col rounded-2xl border-2 ${tier.border} ${tier.gradient} p-6 shadow-sm`}>
                {tier.popular && (<div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-blue-600 px-4 py-1 text-xs font-bold text-white shadow">
                ...
                <ul className="mb-8 flex-1 space-y-3">
                  {tier.features.map((f, fi) => (<li key={fi} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                      <CheckCircle2 className="mt-0.5 h-4 w-4 flex-shrink-0 text-green-500"/>
                      {f}
                    </li>))}
                </ul>
                <Link href={`/${locale}${tier.ctaHref}`} className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-center font-semibold shadow transition ${tier.popular
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-green-100 px-3 py-1 text-sm font-medium text-green-800 dark:bg-green-900 dark:text-green-200">{tr("💚 微信支付")}</div>
              <div className="flex justify-center">
                <img src="https://usedfarmmach-oss.oss-cn-beijing.aliyuncs.com/payment/wechat-qr.png" alt={tr("微信支付二维码")} className="h-56 w-56 rounded-lg border border-gray-200 object-contain p-2 dark:border-gray-700"/>
              </div>
              <p className="mt-3 text-xs text-gray-500">{tr("扫一扫即可付款")}</p>
            </div>

            {/* Alipay */}
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-sm font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-200">{tr("💙 支付宝支付")}</div>
              <div className="flex justify-center">
                <img src="https://usedfarmmach-oss.oss-cn-beijing.aliyuncs.com/payment/alipay-qr.jpg" alt={tr("支付宝支付二维码")} className="h-56 w-56 rounded-lg border border-gray-200 object-contain p-2 dark:border-gray-700"/>
              </div>
              <p className="mt-3 text-xs text-gray-500">{tr("扫一扫即可付款")}</p>
            </div>
          </div>
      ...
      <section className="bg-gray-50 py-12 dark:bg-gray-900">
        ...
        ].map((s, i) => (<div key={i} className="rounded-xl bg-white p-6 text-center shadow-sm dark:bg-gray-800">
      ...
      <section className="py-12">
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <div className="mt-6 flex justify-center gap-4">
            <Link href={`/${locale}/expo/brand-claim`} className="rounded-lg bg-green-600 px-8 py-3 font-semibold text-white shadow-lg transition hover:bg-green-700">
              {tr("免费开通→")}
            </Link>
            <Link href={`/${locale}/expo`} className="rounded-lg border border-gray-300 px-8 py-3 font-semibold text-gray-800 transition hover:bg-gray-50 dark:border-gray-600 dark:text-white dark:hover:bg-gray-800">
              {tr("浏览展会")}
            </Link>
          </div>
        </div>
