...
export default function ProductSelector({ products, selectedProduct, onSelect }: ProductSelectorProps) {
    ...
    return (<div className="space-y-4">
      <div className="relative">
        <label htmlFor="product-select" className="block text-sm font-medium text-gray-700 mb-2">{tr("选择产品")}</label>
        <div className="relative">
          <select id="product-select" className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-colors appearance-none" value={selectedProduct?.id || ""} onChange={(e) => {
      ...
      {/* 搜索结果筛选提示 */}
      {products.length > 10 && (<div className="text-sm text-gray-500 mt-2">{tr("使用下拉菜单浏览全部")}{products.length}{tr("个产品，或直接在菜单中搜索品牌/型号")}</div>)}
    </div>);
}
