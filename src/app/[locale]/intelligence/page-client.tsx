...
import { Globe, TrendingUp, BarChart3, Calendar, ChevronDown, ChevronUp, Lightbulb, Table, ArrowRight, X } from "lucide-react";
...
function IntelCard({ item, locale, actionTipsLabel }: IntelCardProps) {
    ...
    const renderTip = (tip: string) => {
        ...
        while ((match = regex.exec(tip)) !== null) {
            ...
            parts.push(<a key={parts.length} href={match[2]} target="_blank" rel="noopener noreferrer" className="text-accent-600 hover:text-accent-800 underline">
          {match[1]}
        </a>);
    ...
    const renderContent = (content: string) => {
        ...
        return content.split("\n").map((line, i) => {
            ...
            if (line.startsWith("|") && line.endsWith("|")) {
                ...
                return (<div key={i} className="flex text-xs">
            {cells.map((cell, ci) => (<span key={ci} className={`flex-1 px-2 py-1 ${isHeader ? "font-bold text-accent-700 bg-accent-50" : "text-gray-600 border-b border-gray-100"}`}>
    ...
    return (<div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden transition-all">
      {/* 卡片头部（始终可见） */}
      <button onClick={() => setExpanded(!expanded)} className="w-full p-5 text-left transition-colors hover:bg-accent-50/30">
        <div className="flex items-start gap-3">
          <span className="text-2xl flex-shrink-0 mt-0.5">{item.icon}</span>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900 leading-relaxed">{item.text}</p>
            {/* 标签 */}
            {item.tags?.length > 0 && (<div className="mt-2 flex flex-wrap gap-1.5">
                {item.tags.map((tag) => (<span key={tag} className="rounded-full bg-accent-50 px-2 py-0.5 text-[10px] text-accent-600">
                    {tag}
                  </span>))}
              </div>)}
          </div>
      ...
      {/* 展开后的详细内容 */}
      {expanded && item.detailedContent && (<div className="border-t border-gray-100 bg-gray-50/50">
          <div className="p-5">
            ...
            {/* 行动建议 */}
            {item.actionTips && item.actionTips.length > 0 && (<div className="mt-4 rounded-lg border border-accent-100 bg-accent-50 p-4">
                <div className="flex items-center gap-1.5 mb-2">
                  <Lightbulb className="h-4 w-4 text-amber-500"/>
                  <span className="text-xs font-bold text-accent-700">{actionTipsLabel}</span>
                </div>
                <ul className="space-y-1.5">
                  {item.actionTips.map((tip, i) => (<li key={i} className="flex items-start gap-2 text-xs text-gray-700">
...
export default function IntelligencePageClient({ locale }: {
    locale: string;
}) {
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        {loading ? (<div className="flex items-center justify-center py-20 text-gray-400">
            <div className="h-8 w-8 animate-spin rounded-full border-4 border-accent-200 border-t-accent-600"/>
            <span className="ml-3">{t("loading")}</span>
          </div>) : marketData.length === 0 ? (<div className="py-20 text-center text-gray-400">
            <p>{t("empty")}</p>
          </div>) : (<>
        {/* 区域分组 */}
        {allRegions.filter(r => grouped[r]).map((region) => (<section key={region} className="mb-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {grouped[region].map((item, idx) => (<IntelCard key={idx} item={item} locale={locale} actionTipsLabel={t("actionTips")}/>))}
            </div>
          </section>))}
  ...
  const tr = useTr();
  return (<Link key={item.rank} href={`/${locale}/products/${item.id}`} className="flex items-center gap-3 rounded-lg border border-gray-100 p-3 transition-colors hover:border-accent-200 hover:bg-accent-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500">¥{item.price / 10000}{tr("万")}</div>
                  </div>
